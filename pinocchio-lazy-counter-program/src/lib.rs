...
const ACCOUNTS_NEEDED: u64 = 1;
const INCREMENT: u64 = 1;

fn process_instruction(
    mut context: InstructionContext
) -> ProgramResult {
    if context.available() != ACCOUNTS_NEEDED {
        return Err(
            ProgramError::NotEnoughAccountKeys
        );
    };

    let counter_account_info = unsafe { context.next_account_unchecked() };
    if let MaybeAccount::Account(counter_account) = counter_account_info {
        if counter_account.data_len() != COUNTER_SIZE {
            return Err(
                ProgramError::InvalidAccountData
            );
        };

        let counter_account_data: &mut [u8] = &mut counter_account
            .try_borrow_mut_data()
            .unwrap(); 
...
mod test_counter_program {
    ...
    fn get_mollusk_svm() -> Mollusk {
        Mollusk::new(
            ...
            "target/deploy/pinocchio_lazy_counter_program"
        )
    }
